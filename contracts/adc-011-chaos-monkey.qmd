---
title: "Chaos Monkey VHDL Violation Injector"
...
Create a systematic test harness that injects known VHDL hardware violations into working designs to validate the analyzer's detection capabilities. This "chaos monkey" approach ensures comprehensive coverage of both catastrophic and subtle hardware errors.
...
output_directory: "contrib/lzx-chaos"
violation_manifest: "chaos-violations.json"
```

### Output Structure
```
contrib/lzx-chaos/
├── chaos-violations.json          # Master violation catalog
├── enhanced.vhd                   # Enhanced with violation markers
├── contrast-cat1-violation3.vhd   # Category 1, violation 3
├── gamma-cat2-violation1.vhd      # Category 2, violation 1
...
**Location**: `scripts/chaos-monkey.py`
...
- Clear one-to-one mapping violation → file
- Parallel testing capability

**Inline Documentation**: Every injection includes:
```vhdl
-- CHAOS-MONKEY: cat1-violation3
...
When implementing the chaos monkey generator:
...
python scripts/chaos-monkey.py \
  ...
  --manifest chaos-violations.json

# Run analyzer on all variants
stack exec spellcraft -- contrib/lzx-chaos/*.vhd \
  --report chaos-detection-report.json
...
- ✅ Parser achieves 100% success on clean Lumarian corpus
- ✅ Basic Clash violation detection implemented (adc-008)
- ✅ Clock domain tracking functional (adc-003, adc-007)
- ⏳ Violation detection rules defined (to be implemented)

## Postconditions

- 🎯 Comprehensive violation test suite exists
- 🎯 Detection gaps are documented
- 🎯 Regression tests prevent detection loss
- 🎯 Clear roadmap for new detection features
